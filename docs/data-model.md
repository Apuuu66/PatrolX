...
KPI 跨任务历史不新增数据库历史点表。设备 ID 保存在任务元数据（`customer["device_id"]`）；有效趋势点保存在任务输出 `kpi/history/index.jsonl`，索引不保存设备 ID。字段、降级和查询边界见 [025 历史趋势数据模型](../specs/025-kpi-cross-task-trend/data-model.md)。
