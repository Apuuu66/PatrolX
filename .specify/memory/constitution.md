...
普通规则必须声明 `source_patterns`。这些正则表达式必须匹配 `output/<task_id>/` 下的相对文件路径，并定义该规则能够处理的文件范围。规则不得导入、实例化或调用其他规则，不得建立规则间依赖图；普通规则不声明 `inputs[]`，基线模型不引入 artifacts。隐藏的 `pkg.extract.*` 是唯一的解压准备规则。

### 增量重跑

规则结果必须按规则粒度持久化，使得单条规则可以重跑而不影响无关结果。重跑规则时，执行器必须先确保安全幂等解压完成，再按目标规则 `source_patterns` 匹配文件并执行目标规则；随后只替换该规则 JSON，并更新任务摘要和 HTML 报告。巡检逻辑变更时必须递增 `rule_version`。

### 解压先行

任务执行和单规则重跑前必须确保归档解压完成。解压必须安全、分类落位和幂等；嵌套子包必须通过清单和校验和去重，只解压一次。普通规则只读取 `output/<task_id>/` 中匹配到的已解压文件，不得重复解压主包。
...
创建 worktree 前，实现分支必须已包含本功能的全部 Speckit 产物（`spec.md`、`plan.md`、`tasks.md` 及检查清单）并已提交；禁止将未跟踪的规格产物复制到 worktree 中。具体目录、复用和清理规则由 Agent 指南维护。
...
**版本**：2.1.0 | **批准日期**：2026-09-12 | **最后修正**：2026-09-12
