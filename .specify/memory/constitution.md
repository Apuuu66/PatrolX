...
大型功能、契约变更、数据模型演进、调度器变更和复杂交互设计必须走 Speckit；小修复可以省略 Speckit。Speckit 是需求规格、实现方案和任务清单的唯一规划来源；规划产物在进入实现前提交，实现阶段只更新任务完成状态。Superpowers 只负责实现执行、调试、TDD 和完成前验证。默认在主工作区实现；仅在用户要求隔离或并行时使用 worktree。
创建 worktree 前，实现分支必须已包含本功能的全部 Speckit 产物（`spec.md`、`plan.md`、`tasks.md` 及检查清单）并已提交；禁止将未跟踪的规格产物复制到 worktree 中。
...
**版本**：2.2.0 | **批准日期**：2026-09-12 | **最后修正**：2026-09-13
