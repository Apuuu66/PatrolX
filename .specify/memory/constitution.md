...
新功能、新增交互能力或新增巡检能力必须采用“测试 + 展示效果”双验证：

1. 自动化测试必须覆盖核心用户路径、关键契约和主要失败边界。
2. 展示效果用例必须提供可重复运行的代表性样例数据或 fixture，并能验证结果在对应界面、报告或 API 响应中真实可见；前端能力必须包含覆盖完整入口到关键结果的 E2E。
3. 完成定义不得停留在“代码已实现”或“局部函数通过”。基础路径必须按真实用户操作可用；缺少自动化回归或展示效果证据的功能不得进入合入。
...
**版本**：2.8.0 | **批准日期**：2026-09-12 | **最后修正**：2026-09-24
