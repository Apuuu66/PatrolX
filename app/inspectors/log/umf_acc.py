...
inspector = Inspector(
    ...
    rule_version="4.0.0",
    description="检查 UMFAcc 的数据库连接池耗尽与 SCTP 链路错误",
...
def _read_prepared_records(ctx: RuleContext) -> list[dict]:
    path = prepared_dir(ctx, inspector.code) / "umf_acc_records.jsonl"
    if not path.exists():
        return []
...
def _run(ctx: RuleContext) -> object:
    ...
    if ctx.files and not records:
        return make_result(
            ...
            summary="未发现 UMFAcc 日志",
            skip_reason="未发现 UMFAcc 日志",
        )
    if not ctx.files:
        return make_result(
            inspector,
    ...
    if len(pool_records) >= POOL_FAIL_COUNT or len(sctp_records) >= SCTP_FAIL_COUNT:
        ...
        summary = "UMFAcc 存在数据库连接池耗尽" if pool_records else "UMFAcc 存在 SCTP 链路异常"
    elif pool_records or sctp_records:
        status = RuleStatus.WARN
        summary = "UMFAcc 存在需关注日志"
    else:
...
inspector.run = _run
inspector.prepare = PrepareSpec(code="prepare.log.umf_acc", owner_code=inspector.code, run=_prepare)
