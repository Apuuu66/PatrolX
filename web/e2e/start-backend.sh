#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
E2E_DIR="$ROOT_DIR/web/e2e/.tmp"
TASK_ID="task-legacy-e2e"

rm -rf "$E2E_DIR"
mkdir -p "$E2E_DIR/output/$TASK_ID" "$E2E_DIR/uploads/$TASK_ID"
cp "$ROOT_DIR/web/e2e/fixtures/task.json" "$E2E_DIR/output/$TASK_ID/task.json"
touch "$E2E_DIR/uploads/$TASK_ID/legacy.zip"

export PATROLX_OUTPUT_DIR="$E2E_DIR/output"
export PATROLX_UPLOADS_DIR="$E2E_DIR/uploads"
export PATROLX_SQLITE_PATH="$E2E_DIR/patrolx.db"

exec "$ROOT_DIR/.venv/bin/python" -m uvicorn app.main:app --host 127.0.0.1 --port 8010
