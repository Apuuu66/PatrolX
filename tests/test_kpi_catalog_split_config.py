...
from tests.kpi_helpers import KPI_BASE_FILES, kpi_catalog_payload, write_kpi_split_config


def test_settings_use_fixed_kpi_base_paths() -> None:
...
def test_settings_env_override_keeps_base_layout() -> None:
    configured = Settings(kpi_data_dir="/tmp/kpi-data")
    assert configured.kpi_metrics_file == Path("/tmp/kpi-data/base/metrics.json")
    assert configured.kpi_units_file == Path("/tmp/kpi-data/base/units.json")


def test_load_valid_base_catalog(tmp_path: Path) -> None:
    ...
    assert catalog.rules.metric_rules == {}
    assert catalog.rules.thresholds == []
    assert catalog.base_data_version.startswith("sha256:")


@pytest.mark.parametrize("relative", KPI_BASE_FILES)
...
def test_invalid_json_and_duplicate_fields_fail(tmp_path: Path) -> None:
    ...
    data_dir = write_kpi_split_config(tmp_path)
    (data_dir / "base/metrics.json").write_text("{bad", encoding="utf-8")
...
def test_base_data_version_is_fixed_order_content_fingerprint(tmp_path: Path) -> None:
    ...
    combined = b"".join(
        relative.encode("utf-8") + b"\x00" + (first / relative).read_bytes() + b"\x00" for relative in KPI_BASE_FILES
    )
    assert catalog.base_data_version == "sha256:" + hashlib.sha256(combined).hexdigest()

    second = write_kpi_split_config(tmp_path / "second")
    metrics_path = second / "base/metrics.json"
    metrics_path.write_text(metrics_path.read_text(encoding="utf-8").rstrip() + "  \n", encoding="utf-8")
    changed = load_kpi_catalog(second)
    assert changed.base_data_version != catalog.base_data_version


def test_error_locations_for_base_resources(tmp_path: Path) -> None:
    data_dir = write_kpi_split_config(tmp_path)
...
def test_reserved_unit_does_not_affect_base_metrics(tmp_path: Path) -> None:
    ...
    assert catalog.metrics == load_kpi_catalog(write_kpi_split_config(tmp_path / "valid")).metrics
