...
4. **给定**同名且内容一致的包再次进入系统，**当**处理请求完成，**则**系统复用既有任务现场，不创建第二个任务。
5. **给定**同名但内容不同的包再次进入系统，**当**用户尝试处理，**则**系统返回冲突错误且既有任务输入、输出和结果不被修改。
...
规则开发者修改了一条巡检规则并希望在不重跑所有规则的情况下验证它。开发者可以重跑目标规则；如果任务现场尚未解压，系统先完成安全解压。目标规则按自己声明的文件规则读取已解压文件并重新生成结果。已有的无关结果保持可用。

**优先级理由**：快速规则迭代提高巡检质量，但依赖于基线巡检流程已正常工作。

**独立测试**：在巡检已存在后，重跑一条规则并验证仅目标结果被刷新；规则按 `source_patterns` 匹配文件，其他规则结果保持可识别。

**验收场景**：

1. **给定**已完成包含目标规则结果的巡检，**当**用户请求重跑该规则，**则**仅目标规则结果被更新。
2. **给定**目标规则声明了 `source_patterns`，**当**系统重跑该规则，**则**执行器在任务目录中匹配文件并传给规则。
3. **给定**规则没有匹配到任何文件，**当**重跑完成，**则**规则结果为 `skip` 且包含可读原因。
4. **给定**重跑完成，**当**用户查看其他规则结果，**则**这些结果保持不变且可访问。
5. **给定**本地模式后端与前端读取同一输出现场，**当**用户通过 CLI 重跑单条规则，**则**前端刷新任务详情、规则详情和报告预览时展示最新规则结果、摘要统计和报告内容。
...
- 单条规则执行或解析抛出异常；该规则必须产生显式 `error` 结果，`error` 结果不要求输出 metrics 或 findings；执行日志必须包含任务与规则上下文，任务不得中止，其他规则结果不得丢失。
- 两个任务携带相同的省份/运营商/版本等系统元数据；每个任务保持隔离，后续可按元数据比较。
...
- **FR-001**：系统必须接受一个包名唯一的预收集包作为一个巡检任务；`task_id` 必须来自唯一包名，同一包名对应同一任务现场。当同名包再次进入系统时，若包内容校验和一致，必须复用既有任务现场；若包内容校验和不同，必须以冲突错误拒绝处理，不得覆盖既有任务。同名不同内容冲突必须使用统一错误结构返回，在线 API 使用 `409` 状态码和错误码 `package_checksum_conflict`。
- **FR-002**：系统执行巡检不得要求访问被检系统。
- **FR-003**：系统必须按 `task_id` 隔离包数据、巡检上下文、规则结果、日志和报告。
...
- **FR-007**：系统必须先完成安全分类解压，再执行巡检规则；规则之间不得互相调用或共享中间文件。
- **FR-008**：系统必须记录规则结果，包含状态、摘要、适用时的指标、适用时的发现、执行时间和耗时。
- **FR-009**：系统必须支持通过、告警、失败、异常和跳过的规则结果状态。
- **FR-010**：系统必须在规则被跳过时要求可见的原因。
- **FR-011**：系统必须为发现保留包相对来源文件路径和证据，使用户能够将结论追溯回包数据。证据必须截断至安全展示大小，同时保留完整来源文件路径。
- **FR-012**：系统必须提供可读的巡检报告，汇总整体状态、规则结果、发现和建议。
- **FR-013**：系统必须允许用户按任务、系统、规则和发现审查结果。
- **FR-014**：系统必须允许重跑目标规则而不要求无关规则丢失其结果。
- **FR-015**：每条可见巡检规则必须声明 `source_patterns`；系统重跑该规则时必须在任务目录中匹配相对路径，并将匹配文件交给规则处理；无匹配文件时结果必须是带原因的 `skip`。
- **FR-016**：系统必须在同一包通过本地和共享/在线体验处理时保持巡检含义一致。
- **FR-017**：系统必须保留已完成的任务及其巡检证据，直到用户显式删除任务。
- **FR-018**：系统必须在任务被删除时同时移除保留的包输入和生成的巡检输出。
- **FR-019**：系统必须在本地或在线单规则重跑完成后更新目标规则结果、任务摘要和 HTML 报告，使前端刷新后呈现最新结果。
- **FR-020**：系统必须只使用 `task_id` 作为运行现场唯一标识；上传的省份、运营商、产品形态、版本等系统元数据保存为任务字段，不作为目录层级或第二个运行时身份。
- **FR-021**：系统必须通过新增 `/api/v2` 交付本基线契约；在 `/api/v2` 及消费者迁移完成前，不得删除或重定义既有 `/api/v1` 字段。迁移完成后，`/api/v1` 必须进入废弃删除流程，最终从公开契约、实现、测试和生成客户端中移除。

### 关键实体

- **巡检任务**：由一个包名唯一的包触发的一次执行；`task_id` 来自唯一包名，具有生命周期状态、触发源、上传系统元数据、时序和汇总结果计数。
- **系统巡检**：任务内嵌的 1:1 巡检上下文；包含包信息、可选系统/版本元数据、状态和规则摘要，不使用独立运行时主键。
- **巡检规则**：命名的、有版本的巡检能力，具有类别、严重程度、优先级、`source_patterns`、指标输出、状态、摘要和发现。严重程度取值为 `low`、`medium`、`high`、`critical`。
...
- **SC-006**：用户可以从报告中识别整体任务健康状况和至少最重要的可操作发现，无需阅读原始包文件。报告必须优先展示按 `critical`、`high`、`medium`、`low` 排序的发现；同级别发现按规则优先级和规则代码稳定排序。
